...
import { saveStorybook } from "@/lib/storage";
...
export default function Home() {
  ...
  const handleGenerateStorybook = async () => {
    if (sources.filter(s => s.status === "success").length === 0) {
        toast({ title: "No sources available", description: "Add sources to generate storybook." });
        return;
    }

    setIsGeneratingStorybook(true);
    try {
        const res = await fetch("/api/gemini/storybook", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                sources: sources
        ...
        const title = sourceTitles.length > 0
          ? `Story from ${sourceTitles[0]}${sourceTitles.length > 1 ? ` and ${sourceTitles.length - 1} more` : ""}`
          : "Generated Storybook";

        saveStorybook(title, data.pages, sourceTitles);

        toast({ title: "Storybook generated and saved" });
    } catch (error) {
        console.error(error);
        toast({ title: "Failed to generate storybook", variant: "destructive" });
    } finally {
        ...
        <div className={`${mobileTab === "outputs" ? "block" : "hidden"} lg:block lg:col-span-4 h-full overflow-hidden bg-white`}>
          <div className="h-full overflow-y-auto">
            <OutputsPanel
                ...
                isGeneratingStorybook={isGeneratingStorybook}
